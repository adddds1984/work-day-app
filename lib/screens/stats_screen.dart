...
class _StatsScreenState extends State<StatsScreen> {
  ...
  Widget _buildSummaryCard(Object stats) {
    final isMonthly = stats is MonthlyStats;
    final year = isMonthly ? stats.year : (stats as YearlyStats).year;
    final month = isMonthly ? stats.month : null;
    final totalDays = isMonthly ? stats.totalWorkDays : (stats as YearlyStats).totalWorkDays;
    final totalIncome = isMonthly ? stats.totalIncome : (stats as YearlyStats).totalIncome;
    final whiteDays = isMonthly ? stats.whiteShiftDays : (stats as YearlyStats).whiteShiftDays;
    final otDays = isMonthly ? stats.overtimeDays : (stats as YearlyStats).overtimeDays;
    final btDays = isMonthly ? stats.businessTripDays : (stats as YearlyStats).businessTripDays;
    final restDays = isMonthly ? stats.restDays : (stats as YearlyStats).restDays;
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _buildStatItem('\''总天数'\'', '\''\${totalDays}'\'', Colors.white),
          _buildStatItem('\''总收入'\'', '\''¥\${totalIncome.toStringAsFixed(0)}'\'', Colors.yellow[100]!),
        ]),
        const SizedBox(height: 16),
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _buildStatItem('\''白班'\'', '\''\${whiteDays}'\'', WorkDayType.whiteShift.color),
          _buildStatItem('\''加班'\'', '\''\${otDays}'\'', WorkDayType.overtime.color),
          _buildStatItem('\''出差'\'', '\''\${btDays}'\'', WorkDayType.businessTrip.color),
          _buildStatItem('\''休息'\'', '\''\${restDays}'\'', WorkDayType.rest.color),
        ]),
      ]),
    );
  }
