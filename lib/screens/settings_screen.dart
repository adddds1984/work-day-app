...
class _SettingsScreenState extends State<SettingsScreen> {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        SizedBox(width: 100, child: TextField(
          ...
          decoration: InputDecoration(prefixText: '¥ ', border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: type.color)), contentPadding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8)),
          textAlign: TextAlign.right,
          onChanged: (v) {
            final price = double.tryParse(v);
            if (price != null && price >= 0) {
              widget.provider.priceConfig.setPrice(type, price);
              widget.provider.updatePriceConfig(widget.provider.priceConfig);
            }
          },
        )),
      ]),
  ...
  Widget _buildDataAction({required IconData icon, required String title, required String subtitle, Color color = Colors.blue, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
    ...
    for (int year = 2020; year <= now.year; year++) {
      for (int month = 1; month <= 12; month++) {
        ...
        if (records.isNotEmpty) {
          content += '\n--- \\ ---\n';
          for (final r in records) content += '  ¥\\n';
        }
      }
    }
        ...
        ElevatedButton.icon(
          onPressed: () {
            ...
            if (dialogContext.mounted) {
              ...
              ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('已复制到剪贴板')));
            }
          },
  ...
  void _clearAllData() {
    showDialog(context: context, builder: (dialogContext) => AlertDialog(
      ...
      content: const Text('此操作将删除所有工日记录和设置，且无法恢复。确定要继续吗？'),
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            ...
            if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('数据已清空')));
          },
...
class _AboutCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Column(children: [
      Text('记工日', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
      SizedBox(height: 8),
      Text('版本 2.0.0', style: TextStyle(color: Colors.grey)),
      SizedBox(height: 8),
      Text('离线工日记录工具', style: TextStyle(color: Colors.grey)),
      SizedBox(height: 16),
      Text('所有数据保存在本地，无需联网', style: TextStyle(color: Colors.grey, fontSize: 12)),
    ]),
  );
}